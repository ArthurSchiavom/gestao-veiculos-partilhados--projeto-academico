...
CREATE OR REPLACE FUNCTION func_find_minimum_battery_scooter(start_park_id_in IN varchar2, minimum_battery_in IN numeric)
...
CREATE OR REPLACE FUNCTION func_find_highest_charge_scooter(start_park_id_in IN varchar2) 
RETURN electric_scooters%rowtype IS
	scooter electric_scooters%rowtype;
BEGIN
	WITH parked_electric_scooters AS (
...
CREATE OR REPLACE FUNCTION func_find_available_bicycle(start_park_id_in IN varchar2) 
...
CREATE OR REPLACE FUNCTION func_unlock_vehicle(start_time_in IN timestamp, user_email_in IN varchar2, vehicle_id_in in int, 
