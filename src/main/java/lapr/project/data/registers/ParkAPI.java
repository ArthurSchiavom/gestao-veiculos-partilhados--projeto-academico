...
public class ParkAPI {
    ...
    /**
     * Returns a park by it's own coordinates
     *
     * @param lat the latitude of the park
     * @param lon the longitude of the park
     * @return the park in those coordinates
    ...
    /**
     * Finds a scooter that has the highest charge in a given park
     *
     * @param startingParkId the starting park id
     * @return (1) the vehicleDescription
     * (2) null if it couldn't find anything
     * (3) null if there was an sql exception
     */
    ...
    /**
     * Finds a scooter that has the minimum amount of battery for a trip
     *
     * @param startingParkId the starting park description
     * @param minimumBattery the minimum battery(without the extra required 10%, the sql takes care of that)
     * @return (1) the description of the vehicle with the minimum battery
     * (2) null if it can't find anything
     * (3) null if there is an sql exception
     */
    public String fetchScooterWithMinimumBattery(String startingParkId, float minimumBattery) {
        AutoCloseableManager autoCloseableManager = new AutoCloseableManager();
        try {
            CallableStatement cs = dataHandler.prepareCall(
                    "SELECT func_find_minimum_battery_scooter(?, ?) FROM DUAL");
            cs.setString(1, startingParkId);
            cs.setFloat(2, minimumBattery);
